...
const translations: Record<string, Record<Exclude<AppLocale, "zh-CN">, string>> = {
  ...
  "只管理 Airlock 的 HTTP 与 SSH 监听端口。结束进程前会再次核对它仍在监听对应端口，且仅允许结束当前 macOS 用户的进程。": { en: "Manage only Airlock's HTTP and SSH listener ports. Before ending a process, Airlock checks again that it still owns that port and allows only processes owned by the current macOS user.", ja: "Airlock の HTTP / SSH リスナーポートだけを管理します。終了前に対象ポートをまだ待ち受けていることを再確認し、現在の macOS ユーザーが所有するプロセスだけを終了できます。" },
