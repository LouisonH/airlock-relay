...
const translations: Record<string, Record<Exclude<AppLocale, "zh-CN">, string>> = {
  ...
  "窗口控制": { en: "Window controls", ja: "ウィンドウ操作" },
  "最小化窗口": { en: "Minimize window", ja: "ウィンドウを最小化" },
  "最大化窗口": { en: "Maximize window", ja: "ウィンドウを最大化" },
  ...
  "管理端口": { en: "Manage ports", ja: "ポートを管理" },
  "本地端口管理": { en: "Local port management", ja: "ローカルポート管理" },
  "端口冲突处置": { en: "Resolve port conflicts", ja: "ポート競合の解決" },
  "只管理 Airlock 的 HTTP 与 SSH 监听端口。结束进程前会再次核对它仍在监听对应端口，且仅允许结束当前系统用户的进程。": { en: "Manage only Airlock's HTTP and SSH listener ports. Before ending a process, Airlock checks again that it still owns that port and allows only processes owned by the current system user.", ja: "Airlock の HTTP / SSH リスナーポートだけを管理します。終了前に対象ポートをまだ待ち受けていることを再確認し、現在のシステムユーザーが所有するプロセスだけを終了できます。" },
  "监听端口": { en: "Listener ports", ja: "リスナーポート" },
  "本地监听端口": { en: "Local listener ports", ja: "ローカルリスナーポート" },
  "端口被占用时，可切换到其他非特权端口，或结束当前用户的占用进程。": { en: "When a port is occupied, switch to another unprivileged port or end the current user's occupying process.", ja: "ポートが使用中の場合は、別の非特権ポートに切り替えるか、現在のユーザーが所有する使用中のプロセスを終了できます。" },
  "使用 1024-65535 的不同端口；保存后本地核心会重启，失败时自动恢复原设置。": { en: "Use two different ports from 1024 to 65535. Saving restarts the local core and restores the previous settings if startup fails.", ja: "1024-65535 の異なる 2 つのポートを使用します。保存後はローカルコアが再起動し、起動に失敗した場合は元の設定に戻ります。" },
  "检查端口占用": { en: "Check port usage", ja: "ポート使用状況を確認" },
  "固定 URL 与 LLM 路由": { en: "Pinned URL and LLM routes", ja: "固定 URL と LLM ルート" },
  "SSH 用户名映射": { en: "SSH username mappings", ja: "SSH ユーザー名マッピング" },
  "请输入两个不同的 1024-65535 端口。": { en: "Enter two different ports from 1024 to 65535.", ja: "1024-65535 の異なる 2 つのポートを入力してください。" },
  "保存会重启本地核心；已有路由和受保护凭据不会改变。": { en: "Saving restarts the local core; existing routes and protected credentials are unchanged.", ja: "保存するとローカルコアが再起動します。既存のルートと保護された認証情報は変更されません。" },
  "当前端口设置已生效。": { en: "The current port settings are active.", ja: "現在のポート設定は有効です。" },
  "正在应用": { en: "Applying", ja: "適用中" },
  "保存端口": { en: "Save ports", ja: "ポートを保存" },
  "占用当前端口的进程": { en: "Processes using the current ports", ja: "現在のポートを使用中のプロセス" },
  "只显示当前用户的监听进程；Airlock 自己的本地核心不会出现在列表中。": { en: "Only listener processes owned by the current user are shown. Airlock's own local core is excluded.", ja: "現在のユーザーが所有する待ち受けプロセスだけを表示します。Airlock 自身のローカルコアは一覧に表示されません。" },
  "正在检查端口占用": { en: "Checking port usage", ja: "ポート使用状況を確認中" },
  "没有发现可结束的占用进程": { en: "No terminable process is using these ports", ja: "終了可能なポート使用プロセスは見つかりませんでした" },
  "结束": { en: "End", ja: "終了" },
  "确认结束该进程？": { en: "End this process?", ja: "このプロセスを終了しますか？" },
  "将向": { en: "Airlock will send", ja: "Airlock は" },
  "发送正常结束请求。Airlock 不会使用强制终止。": { en: "a normal termination request. Airlock will not force-kill it.", ja: "に通常の終了リクエストを送ります。強制終了は行いません。" },
  "正在结束": { en: "Ending", ja: "終了中" },
  "确认结束": { en: "Confirm end", ja: "終了を確認" },
  ...
  "本地核心未能在 20 秒内准备就绪。请重试；若问题持续，请检查 127.0.0.1:4768 或 127.0.0.1:4770。": { en: "The local core was not ready within 20 seconds. Retry; if this persists, check 127.0.0.1:4768 or 127.0.0.1:4770.", ja: "ローカルコアは 20 秒以内に準備できませんでした。再試行し、問題が続く場合は 127.0.0.1:4768 または 127.0.0.1:4770 を確認してください。" },
  ...
  "默认仅保留脱敏类别、结果与耗时；开启命令审计后，可在详情中查看完整 SSH 命令": { en: "Activity is sanitized by default; when command audit is enabled, full SSH commands can be viewed in details", ja: "アクティビティは既定でサニタイズされます。コマンド監査を有効にすると、詳細で完全な SSH コマンドを確認できます" },
  ...
  "查看详情": { en: "View details", ja: "詳細を表示" },
  "查看活动详情": { en: "View activity details", ja: "アクティビティの詳細を表示" },
  "活动详情": { en: "Activity details", ja: "アクティビティ詳細" },
  "事件类型": { en: "Event type", ja: "イベント種別" },
  "SSH 动作": { en: "SSH action", ja: "SSH アクション" },
  "请求": { en: "Request", ja: "リクエスト" },
  "完整动作": { en: "Full action", ja: "完全なアクション" },
  "复制内容": { en: "Copy content", ja: "内容をコピー" },
  "复制事件 ID": { en: "Copy event ID", ja: "イベント ID をコピー" },
  "仅在路由启用命令记录时保存完整命令": { en: "Full commands are stored only when command recording is enabled for the route", ja: "完全なコマンドは、ルートでコマンド記録が有効な場合のみ保存されます" },
  "已脱敏，不包含目标、凭据或请求正文": { en: "Sanitized; excludes targets, credentials, and request bodies", ja: "サニタイズ済み。対象、認証情報、リクエスト本文は含まれません" },
  ...
  "凭据管理器": { en: "Credential Manager", ja: "資格情報マネージャー" },
  "Windows 凭据管理器": { en: "Windows Credential Manager", ja: "Windows 資格情報マネージャー" },
  "Secret Service": { en: "Secret Service", ja: "Secret Service" },
  "ACL 文件": { en: "ACL-protected file", ja: "ACL 保護ファイル" },
  "账户与文件权限隔离": { en: "Account and file-permission isolation", ja: "アカウントとファイル権限による分離" },
  "凭据管理器 · 仅本机": { en: "Credential Manager · this device only", ja: "資格情報マネージャー · このデバイスのみ" },
  "Secret Service · 仅本机": { en: "Secret Service · this device only", ja: "Secret Service · このデバイスのみ" },
  "ACL 文件 · 仅本机": { en: "ACL file · this device only", ja: "ACL ファイル · このデバイスのみ" },
  "ACL 文件 · 局域网": { en: "ACL file · private LAN", ja: "ACL ファイル · プライベート LAN" },
  "适合长期保存高价值凭据。Secret 由 Windows 凭据管理器加密并控制访问，系统可能要求验证登录凭据。": { en: "For long-lived high-value credentials. Secrets are encrypted and access-controlled by Windows Credential Manager; the system may ask you to verify your sign-in.", ja: "長期保存する高価値の認証情報向けです。Secret は Windows 資格情報マネージャーが暗号化・アクセス制御し、サインイン確認を求められる場合があります。" },
  "适合长期保存高价值凭据。Secret 由桌面 Secret Service 加密并控制访问，系统可能要求解锁钥匙串。": { en: "For long-lived high-value credentials. Secrets are encrypted and access-controlled by the desktop Secret Service; the system may ask you to unlock the keyring.", ja: "長期保存する高価値の認証情報向けです。Secret はデスクトップの Secret Service が暗号化・アクセス制御し、キーリングのロック解除を求められる場合があります。" },
  "默认方案。启动时不读取系统钥匙串，不弹授权框；Secret 未加密，由当前账户和文件权限隔离。": { en: "Default profile. It does not read the system keychain at startup or show an authorization prompt. Secrets are not encrypted and rely on the current account and file permissions.", ja: "既定のプロファイルです。起動時にシステムキーチェーンを読み取らず、認証ダイアログも表示しません。Secret は暗号化されず、現在のアカウントとファイル権限で分離されます。" },
  ...
  "Windows 决定何时显示凭据输入框，Airlock 不能绕过该系统授权。": { en: "Windows decides when to show a credential prompt; Airlock cannot bypass this system authorization.", ja: "資格情報ダイアログを表示するタイミングは Windows が決定し、Airlock がこのシステム認証を回避することはできません。" },
  "系统决定何时要求解锁 Secret Service 钥匙串，Airlock 不能绕过该授权。": { en: "The system decides when to ask you to unlock the Secret Service keyring; Airlock cannot bypass that authorization.", ja: "Secret Service キーリングのロック解除を求めるタイミングはシステムが決定し、Airlock はその認証を回避できません。" },
  "Secret 仅由当前账户与 0600 文件权限隔离；同账户的其他进程可能读取。": { en: "Secrets rely only on the current account and 0600 file permissions; other processes under the same account may read them.", ja: "Secret は現在のアカウントと 0600 ファイル権限だけで分離され、同じアカウントの別プロセスから読み取られる可能性があります。" },
  "Secret 仅由当前 Windows 账户与 ACL 文件权限隔离；同账户的其他进程可能读取。": { en: "Secrets rely only on the current Windows account and ACL file permissions; other processes under the same account may read them.", ja: "Secret は現在の Windows アカウントと ACL ファイル権限だけで分離され、同じアカウントの別プロセスから読み取られる可能性があります。" },
  ...
  "为什么调试包更容易弹出系统凭据框？": { en: "Why do development builds trigger system credential prompts more often?", ja: "開発ビルドでシステムの資格情報確認が増える理由" },
  "为什么调试包更容易弹出系统钥匙串？": { en: "Why do development builds trigger system keyring prompts more often?", ja: "開発ビルドでシステムのキーリング確認が増える理由" },
  ...
  "本地开发包为未签名或自签名构建；每次重建后，Windows 可能把新的 airlockd 视为不同程序并重新验证凭据管理器访问。正式稳定签名可减少询问，但系统仍保留最终授权决定。": { en: "Local development builds are unsigned or self-signed. After each rebuild, Windows may treat the new airlockd as a different program and verify Credential Manager access again. Stable production signing can reduce prompts, but the system retains the final authorization decision.", ja: "ローカル開発ビルドは未署名または自己署名です。再ビルド後、Windows が新しい airlockd を別のプログラムとみなし、資格情報マネージャーへのアクセスを再確認する場合があります。安定した正式署名で確認回数は減らせますが、最終的な認証判断はシステムが保持します。" },
  "Airlock 使用桌面 Secret Service（如 GNOME Keyring / KWallet）。系统要求解锁时，Airlock 不能绕过；正式签名不影响该授权决定。": { en: "Airlock uses the desktop Secret Service (such as GNOME Keyring or KWallet). When the system asks to unlock it, Airlock cannot bypass that; production signing does not change the authorization decision.", ja: "Airlock はデスクトップの Secret Service（GNOME Keyring / KWallet など）を使用します。システムがロック解除を求めた場合、Airlock はそれを回避できません。正式署名によって認証判断が変わることはありません。" },
  ...
  "标准模式启动时不会读取系统钥匙串": { en: "Standard mode does not read the system keychain at startup", ja: "標準モードは起動時にシステムキーチェーンを読み取りません" },
  ...
  "命名管道 · 仅当前用户": { en: "Named pipe · current user only", ja: "名前付きパイプ · 現在のユーザー専用" },
  ...
  "ACL · 不包含明文本地密码": { en: "ACL · no plaintext local password", ja: "ACL · 平文のローカルパスワードを含みません" },
  ...
  "该模式接近远程命令执行权限。保存时还会出现一次 Windows 原生风险确认。": { en: "This is close to remote command execution access. Saving requires an additional native Windows risk confirmation.", ja: "リモートコマンド実行に近い権限です。保存時に Windows ネイティブのリスク確認がもう一度表示されます。" },
  "该模式接近远程命令执行权限。保存时还会出现一次系统原生风险确认。": { en: "This is close to remote command execution access. Saving requires an additional native system risk confirmation.", ja: "リモートコマンド実行に近い権限です。保存時にシステムネイティブのリスク確認がもう一度表示されます。" },
  ...
  "完整命令保存在本机受保护审计文件，参数可能包含敏感内容。": { en: "Full commands are stored in a protected local audit file; arguments may contain sensitive data.", ja: "完全なコマンドを保護されたローカルの監査ファイルに保存します。引数に機密情報が含まれる可能性があります。" },
  ...
  "Shell、PTY 与端口转发仍拒绝；SFTP 由独立开关控制。命令拥有上游账号可访问的数据与操作权限。": { en: "Shell, PTY, and port forwarding stay denied; SFTP is controlled separately. Commands retain the data and operation permissions of the upstream account.", ja: "Shell、PTY、ポート転送は拒否されたままで、SFTP は独立した設定で制御されます。コマンドは上流アカウントのデータと操作権限を使用できます。" },
  "允许 SFTP 文件传输（高风险）": { en: "Allow SFTP file transfer (high risk)", ja: "SFTP ファイル転送を許可（高リスク）" },
  "允许列出、读取、写入、重命名和删除上游账号可访问的文件。请使用专用低权限账号；Shell、PTY、端口转发及其他子系统仍拒绝。": { en: "Allows listing, reading, writing, renaming, and deleting files available to the upstream account. Use a dedicated least-privilege account; Shell, PTY, port forwarding, and other subsystems remain denied.", ja: "上流アカウントがアクセスできるファイルの一覧、読み取り、書き込み、名前変更、削除を許可します。専用の最小権限アカウントを使用してください。Shell、PTY、ポート転送、その他のサブシステムは拒否されます。" },
  "允许交互式 Shell（高风险）": { en: "Allow interactive shell (high risk)", ja: "対話型シェルを許可（高リスク）" },
  "允许交互式 Shell": { en: "Allow interactive shell", ja: "対話型シェルを許可" },
  "PuTTY、ssh 等登录后直接进入上游交互式 Shell；上游凭据仍由 Airlock 注入，调用方只输入本地密码。适合 su、菜单式程序等需要交互的受信任映射。": { en: "PuTTY, ssh, and other clients enter the upstream interactive shell directly after login; Airlock still injects the upstream credentials, so callers only enter the local password. Suited to trusted mappings that need interaction such as su or menu-driven programs.", ja: "PuTTY や ssh などのクライアントは、ログイン後に上流の対話型シェルへ直接入ります。上流の認証情報は Airlock が注入するため、呼び出し元はローカルパスワードだけを入力します。su やメニュー型プログラムなど、対話が必要な信頼できるマッピング向けです。" },
  "新建 SSH 路由默认允许交互式 Shell": { en: "Enable interactive shell by default for new SSH routes", ja: "新しい SSH ルートではデフォルトで対話型シェルを許可" },
  "只影响新建路由的默认勾选；每个路由仍可在创建或编辑时单独调整，且需要开放所有命令。": { en: "Only affects the default checkbox for new routes; each route can still be adjusted when created or edited, and it requires all commands to be allowed.", ja: "新しいルートのデフォルトのチェックのみに影響します。各ルートは作成時または編集時に個別に変更でき、すべてのコマンド許可が必要です。" },
  "默认开启": { en: "Enabled by default", ja: "デフォルトで有効" },
  "默认关闭": { en: "Disabled by default", ja: "デフォルトで無効" },
  "端口转发仍拒绝；SFTP 与交互式 Shell 由独立开关控制。命令拥有上游账号可访问的数据与操作权限。": { en: "Port forwarding remains denied; SFTP and the interactive shell are controlled by separate switches. Commands retain the data and operation permissions of the upstream account.", ja: "ポート転送は拒否されたままで、SFTP と対話型シェルは個別のスイッチで制御されます。コマンドは上流アカウントのデータと操作権限を使用できます。" },
  "现代 OpenSSH 的 scp 默认使用 SFTP。启用后可列出、读取、写入、重命名和删除上游账号可访问的文件；请使用专用低权限账号。": { en: "Modern OpenSSH scp uses SFTP by default. Enabling this permits listing, reading, writing, renaming, and deleting files available to the upstream account; use a dedicated least-privilege account.", ja: "最新の OpenSSH の scp は既定で SFTP を使用します。有効にすると、上流アカウントがアクセスできるファイルの一覧、読み取り、書き込み、名前変更、削除が可能になります。専用の最小権限アカウントを使用してください。" },
