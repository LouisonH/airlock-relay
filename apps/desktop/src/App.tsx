...
const previewRoutes: RouteSummary[] = isTauri ? [] : [
  { id: "release", name: "Release downloads", alias: "release", kind: "HTTP", status: "enabled", localEndpoint: "127.0.0.1:4768/r/release", permissionSummary: "GET, HEAD · Range", egress: "Direct", health: "healthy", lastUsed: "2 分钟前", currentConnections: 1, allowAllCommands: false, recordCommands: false, allowSftp: false, allowInteractiveShell: false },
  { id: "models", name: "Model gateway", alias: "models", kind: "LLM", status: "disabled", localEndpoint: "http://127.0.0.1:4768/r/models", permissionSummary: "OpenAI · 3 models · output ≤ 8192 · 60/min · 4 concurrent", egress: "Proxy", health: "unknown", lastUsed: "从未", currentConnections: 0, allowAllCommands: false, recordCommands: false, allowSftp: false, allowInteractiveShell: false, provider: "openai", allowedModels: ["gpt-5.2", "gpt-5.2-codex", "gpt-5.1"], maxOutputTokens: 8192, requestsPerMinute: 60, maxConcurrent: 4, trackUsage: true, totalRequests: 128, inputTokens: 184320, outputTokens: 42670 },
  { id: "build", name: "Release builder", alias: "build", localUsername: "builder", kind: "SSH", status: "enabled", localEndpoint: "builder@127.0.0.1:4770", permissionSummary: "all exec commands · high risk · recorded", egress: "Auto", health: "healthy", lastUsed: "刚刚", currentConnections: 0, allowAllCommands: true, recordCommands: true, allowSftp: false, allowInteractiveShell: false, allowedCommand: "" },
];
...
export default function App() {
  ...
  const [sshInteractiveDefault, setSSHInteractiveDefault] = useState<boolean>(() => localStorage.getItem("airlock.ssh.interactive.default") === "1");
  ...
  const updateSSHInteractiveDefault = (value: boolean) => {
    setSSHInteractiveDefault(value);
    localStorage.setItem("airlock.ssh.interactive.default", value ? "1" : "0");
  };
  ...
  const updateSSHPolicy = async (name: string, localUsername: string, allowedCommand: string, allowAllCommands: boolean, allowAllConfirmed: boolean, recordCommands: boolean, allowSftp: boolean, allowInteractiveShell: boolean, authenticationTimeoutSeconds: number, egress: RouteSummary["egress"], keywordReplacements: KeywordReplacement[]) => {
    if (!policyRoute) return;
    try {
      const routes = isTauri
        ? await invoke<RouteSummary[]>("set_ssh_policy", { alias: policyRoute.alias, name, localUsername, allowedCommand, allowAllCommands, allowAllConfirmed, recordCommands, allowSftp, allowInteractiveShell, authenticationTimeoutSeconds, egress }).then(() => invoke<RouteSummary[]>("set_ssh_keyword_replacements", { alias: policyRoute.alias, replacements: keywordReplacements }))
        : control.routes.map((route) => route.alias === policyRoute.alias ? {
          ...
          egress,
          permissionSummary: `${allowAllCommands ? "all exec commands · high risk" : "1 exact command · stdin denied"}${recordCommands ? " · recorded" : ""}${allowSftp ? " · SFTP high risk" : ""}${allowInteractiveShell ? " · interactive shell" : ""}${keywordReplacements.length ? ` · ${keywordReplacements.length} rewrite rules` : ""}`,
        } : route);
      setControl((current) => ({ ...current, routes }));
    ...
    <div className="app-shell">
      ...
      <main className="workspace">
        ...
        <div className="page-content" key={page}>
          ...
          {page === "settings" && <SettingsPage platform={platform} language={language} onLanguage={setLanguage} theme={theme} onTheme={setTheme} accent={accent} onAccent={setAccent} density={density} onDensity={setDensity} motion={motion} onMotion={setMotion} refreshInterval={refreshInterval} onRefreshInterval={setRefreshInterval} connected={control.connected} proxyConfigured={control.proxyConfigured} proxyTesting={proxyTesting} sshReady={control.sshReady} securitySettings={control.securitySettings} onSecuritySettings={applySecuritySettings} sshInteractiveDefault={sshInteractiveDefault} onSSHInteractiveDefault={updateSSHInteractiveDefault} onConfigureProxy={configureProxy} onClearProxy={clearProxy} onTestProxy={testProxyHealth} updateCheck={updateCheck} checkingUpdate={checkingUpdate} onCheckUpdates={checkUpdates} onOpenGuide={() => setPage("guide")} onManagePorts={() => setPortManagerOpen(true)} />}
        </div>
      </main>
      ...
      {editorOpen && <RouteEditor initialKind={editorKind} routes={control.routes} connected={control.connected} proxyConfigured={control.proxyConfigured} sshReady={control.sshReady} defaultInteractiveShell={sshInteractiveDefault} onClose={() => setEditorOpen(false)} onCreated={(route) => setControl((current) => ({ ...current, routes: [...current.routes.filter((item) => item.id !== route.id), route] }))} onActivate={checkHealthAndEnableRoute} onError={setNotice} />}
    </div>
  );
}
...
function SettingsPage({ platform, language, onLanguage, theme, onTheme, accent, onAccent, density, onDensity, motion, onMotion, refreshInterval, onRefreshInterval, connected, proxyConfigured, proxyTesting, sshReady, securitySettings, onSecuritySettings, sshInteractiveDefault, onSSHInteractiveDefault, onConfigureProxy, onClearProxy, onTestProxy, updateCheck, checkingUpdate, onCheckUpdates, onOpenGuide, onManagePorts }: { platform: PlatformInfo; language: LocalePreference; onLanguage: (language: LocalePreference) => void; theme: ThemePreference; onTheme: (theme: ThemePreference) => void; accent: AccentTheme; onAccent: (accent: AccentTheme) => void; density: DensityPreference; onDensity: (density: DensityPreference) => void; motion: MotionPreference; onMotion: (motion: MotionPreference) => void; refreshInterval: RefreshInterval; onRefreshInterval: (interval: RefreshInterval) => void; connected: boolean; proxyConfigured: boolean; proxyTesting: boolean; sshReady: boolean; securitySettings: SecuritySettings; onSecuritySettings: (settings: SecuritySettings) => Promise<void>; sshInteractiveDefault: boolean; onSSHInteractiveDefault: (value: boolean) => void; onConfigureProxy: () => void; onClearProxy: () => void; onTestProxy: () => void; updateCheck?: UpdateCheckResult; checkingUpdate: boolean; onCheckUpdates: () => Promise<void>; onOpenGuide: () => void; onManagePorts: () => void }) {
...
function SSHPolicyEditor({ route, platform, testing, onClose, onSave, onUpdateHost, onRotateCredential, onTest, onAddHost, onDelete }: { route: RouteSummary; platform: PlatformInfo; testing: boolean; onClose: () => void; onSave: (name: string, localUsername: string, allowedCommand: string, allowAllCommands: boolean, allowAllConfirmed: boolean, recordCommands: boolean, allowSftp: boolean, allowInteractiveShell: boolean, authenticationTimeoutSeconds: number, egress: RouteSummary["egress"], keywordReplacements: KeywordReplacement[]) => Promise<void>; onUpdateHost: (input: SSHHostUpdateInput) => Promise<void>; onRotateCredential: (localPassword: string) => Promise<SSHRouteCreationResult>; onTest: () => Promise<boolean>; onAddHost: () => void; onDelete: () => void }) {
  ...
  const [allowInteractive, setAllowInteractive] = useState(route.allowInteractiveShell);
  ...
  const save = async () => {
    setBusy("save");
    setError(undefined);
    try { await onSave(name.trim(), localUsername, allowedCommand, allowAll, allowAllConfirmed, recordCommands, allowSftp, allowInteractive, authenticationTimeoutSeconds, egress, keywordReplacements); }
  ...
  return <Modal title={translate(`SSH 映射管理 · ${route.name}`)} className="modal-wide ssh-manager-modal" onClose={busy ? () => undefined : onClose}>
    <div className="ssh-manager">
      ...
      <div className="policy-options" role="radiogroup" aria-label="SSH 命令范围">
        <button className={!allowAll ? "selected" : ""} role="radio" aria-checked={!allowAll} onClick={() => { setAllowAll(false); setAllowAllConfirmed(false); setAllowInteractive(false); }}><ShieldCheck size={17} /><span><strong>指定命令</strong><small>只开放一个完整 exec 命令</small></span></button>
        <button className={allowAll ? "selected risk" : "risk"} role="radio" aria-checked={allowAll} onClick={() => { setAllowAll(true); setAllowAllConfirmed(route.allowAllCommands); }}><AlertTriangle size={17} /><span><strong>所有命令</strong><small>任意 exec；交互式 Shell 由独立开关控制</small></span></button>
      </div>
      ...
      {allowAll && <label className={`audit-toggle interactive-toggle ${allowInteractive ? "enabled" : ""}`}><input type="checkbox" checked={allowInteractive} onChange={(event) => setAllowInteractive(event.target.checked)} /><span><strong>{translate("允许交互式 Shell（高风险）")}</strong><small>{translate("PuTTY、ssh 等登录后直接进入上游交互式 Shell；上游凭据仍由 Airlock 注入，调用方只输入本地密码。适合 su、菜单式程序等需要交互的受信任映射。")}</small></span></label>}
      </section>
...
function RouteEditor({ initialKind, routes, connected, proxyConfigured, sshReady, defaultInteractiveShell, onClose, onCreated, onActivate, onError }: { initialKind: RouteKind; routes: RouteSummary[]; connected: boolean; proxyConfigured: boolean; sshReady: boolean; defaultInteractiveShell: boolean; onClose: () => void; onCreated: (route: RouteSummary) => void; onActivate: (alias: string) => Promise<boolean>; onError: (message: string) => void }) {
  ...
  const [allowInteractiveShell, setAllowInteractiveShell] = useState(defaultInteractiveShell);
    ...
    try {
      ...
      if (isTauri) {
        if (kind === "SSH") {
          ...
          const result = await invoke<SSHRouteCreationResult>("create_ssh_route", { input: { name: name.trim(), alias, localUsername, address: sshEndpoint, username: sshUsername, password: sshPassword, localPassword: localCredentialMode === "custom" ? localPassword : "", expectedHostKey: sshProbe.hostKey, egress, allowedCommand, allowAllCommands, allowAllConfirmed, recordCommands, allowSftp, allowInteractiveShell, authenticationTimeoutSeconds: sshAuthenticationTimeoutSeconds } });
          route = result.route;
  ...
  return <div className="editor-overlay" role="dialog" aria-modal="true" aria-label="新增路由"><div className="editor-panel">
    ...
    <div className="editor-body" key={step}>
      {step === 1 && <>
        ...
        {kind === "SSH" && <div className="route-policy-strip"><span>命令权限</span><div className="policy-segmented"><button className={!allowAllCommands ? "selected" : ""} onClick={() => { setAllowAllCommands(false); setAllowAllConfirmed(false); setAllowInteractiveShell(false); }}><ShieldCheck size={14} />指定命令</button><button className={allowAllCommands ? "selected risk" : "risk"} onClick={() => { setAllowAllCommands(true); setAllowAllConfirmed(false); }}><AlertTriangle size={14} />所有命令</button></div><label className="compact-check"><input type="checkbox" checked={recordCommands} onChange={(event) => setRecordCommands(event.target.checked)} />记录命令</label>{allowAllCommands && <label className="compact-check"><input type="checkbox" checked={allowInteractiveShell} onChange={(event) => setAllowInteractiveShell(event.target.checked)} />{translate("允许交互式 Shell")}</label>}</div>}
        ...
        {kind === "SSH" && allowAllCommands && <div className="inline-warning"><TriangleAlert size={15} />所有命令是高风险能力；交互式 Shell 可单独开启，仅建议用于受信任映射。创建时需要在 Airlock 内明确确认。</div>}
	  ...
	  {step === 2 && kind === "SSH" && <div className="ssh-secure-entry">
          ...
          {allowAllCommands && <label className="risk-consent"><input type="checkbox" checked={allowAllConfirmed} onChange={(event) => setAllowAllConfirmed(event.target.checked)} /><TriangleAlert size={16} /><span><strong>{translate("确认开放所有非交互 exec 命令")}</strong><small>{translate("端口转发仍拒绝；SFTP 与交互式 Shell 由独立开关控制。命令拥有上游账号可访问的数据与操作权限。")}</small></span></label>}
