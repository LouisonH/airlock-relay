...
Airlock 的官方 npm 安装入口。它包含经过 SHA-256 校验的 Airlock v0.1.4
macOS 安装镜像；`npm install` 本身不会静默下载、启动应用或修改系统设置。

> v0.1.4 仅支持 macOS 12+ 与 Apple Silicon。应用采用 ad-hoc 签名，尚未经过
> Apple 公证；已完成维护者执行的生产就绪安全审计，但尚未经过独立第三方审计。
...
[安装与 Gatekeeper 说明](https://github.com/LouisonH/airlock-relay/blob/v0.1.4/docs/installation.zh-CN.md)。
...
SHA-256-verified Airlock v0.1.4 macOS disk image. `npm install` itself does not
download or launch software, or change system settings.

> v0.1.4 supports Apple Silicon Macs running macOS 12 or newer. The application
> is ad-hoc signed and is not Apple-notarized. A maintainer-run production-readiness
> security audit is complete; an independent third-party audit is not.
...
[installation and Gatekeeper guide](https://github.com/LouisonH/airlock-relay/blob/v0.1.4/docs/installation.md)
before first launch.
...
contract for build tooling. Only macOS arm64 is marked as released in v0.1.4;
