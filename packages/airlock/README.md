...
Airlock 的官方 npm 安装入口。它包含经过 SHA-256 校验的 Airlock v0.1.3
macOS 安装镜像；`npm install` 本身不会静默下载、启动应用或修改系统设置。

> v0.1.3 仅支持 macOS 12+ 与 Apple Silicon。应用采用 ad-hoc 签名，尚未经过
...
[安装与 Gatekeeper 说明](https://github.com/LouisonH/airlock-relay/blob/v0.1.3/docs/installation.zh-CN.md)。
...
SHA-256-verified Airlock v0.1.3 macOS disk image. `npm install` itself does not
download or launch software, or change system settings.

> v0.1.3 supports Apple Silicon Macs running macOS 12 or newer. The application
...
[installation and Gatekeeper guide](https://github.com/LouisonH/airlock-relay/blob/v0.1.3/docs/installation.md)
before first launch.
...
contract for build tooling. Only macOS arm64 is marked as released in v0.1.3;
planned targets never resolve to an installer until an artifact and checksum are
published.

## Integrity

Bundled asset: `Airlock_0.1.3_aarch64.dmg`
