...
Airlock v0.1.4 支持运行 macOS 12 Monterey 或更高版本的 Apple Silicon Mac。
首版暂不提供 Intel、Windows 或 Linux 安装包。

## 核验下载

从 [v0.1.4 Release](https://github.com/LouisonH/airlock-relay/releases/tag/v0.1.4) 下载：

- `Airlock_0.1.4_aarch64.dmg`：常规安装包
- `Airlock_0.1.4_aarch64.app.zip`：便携压缩包
- `SHA256SUMS-v0.1.4.txt`：完整性校验值

在下载目录执行：

```bash
shasum -a 256 -c SHA256SUMS-v0.1.4.txt
```
...
该命令校验 npm 包内的 DMG、将应用安装到当前用户的 `~/Applications`，并在完成后打开
Airlock。它不会上传本机路由或凭据，也不会终止其他 Airlock 实例。

```bash
npm install -g airlock-relay && airlock-installer install --open
```

已有可用的 Airlock.app 会被原子更新；如旧包不完整，需在确认路径后添加 `--force`。

### 手动安装

1. 打开 `Airlock_0.1.4_aarch64.dmg`。
...
v0.1.4 已进行 ad-hoc 签名，可检查应用包结构完整性，但没有 Apple Developer
...
开发环境需要 Go 1.25+、Node.js 20+、Rust/Cargo 及 Tauri 2 的 macOS 依赖：
